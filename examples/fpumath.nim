...
proc mtxSRT*(outResult: var Mat4; sx: float32; sy: float32; sz: float32; ax: float32;
            ay: float32; az: float32; tx: float32; ty: float32; tz: float32) {.inline.} =
  var sx: float32 = fsin(ax)
  var cx: float32 = fcos(ax)
  var sy: float32 = fsin(ay)
  var cy: float32 = fcos(ay)
  var sz: float32 = fsin(az)
  var cz: float32 = fcos(az)
  var sxsz: float32 = sx * sz
  var cycz: float32 = cy * cz
  outResult[0] = sx * (cycz - sxsz * sy)
  outResult[1] = sx * - (cx * sz)
  outResult[2] = sx * (cz * sy + cy * sxsz)
  outResult[3] = 0.0'f32
  outResult[4] = sy * (cz * sx * sy + cy * sz)
  outResult[5] = sy * cx * cz
  outResult[6] = sy * (sy * sz - cycz * sx)
  outResult[7] = 0.0'f32
  outResult[8] = sz * - (cx * sy)
  outResult[9] = sz * sx
  outResult[10] = sz * cx * cy
  outResult[11] = 0.0'f32
  outResult[12] = tx
  outResult[13] = ty
  outResult[14] = tz
...
proc vec4MulMtx*(vec: Vec4; mat: Mat4): Vec4 {.inline.} =
...
proc mtxMul*(outResult: var Mat4; a: Mat4; b: Mat4) {.inline.} =
    outResult[0..3] = vec4MulMtx(cast[Vec4](a[0..3]), b)
    outResult[4..7] = vec4MulMtx(cast[Vec4](a[4..7]), b)
    outResult[8..11] = vec4MulMtx(cast[Vec4](a[8..11]), b)
    outResult[12..15] = vec4MulMtx(cast[Vec4](a[12..15]), b)
...
proc calcLinearFit3D*(outResult: var Vec3; points: pointer; stride: int;
  ...
  var mtx: Mat4
  fill(mtx, 0.0'f32)
  mtx[0..8] = [sumXX, sumXY, sumX, sumXY, sumYY, sumY, sumX, sumY,
                          float(numPoints)]
  var invMtx: Mat4
